fn make_new_admin(aid: u64, uid: u64, gid: u64) {
...
fn remove_admin_rights(aid:u64, id: u64, gid: u64) {
    for cid in data_state.user_in_group {
        if cid.uid == aid && cid.gid == gid {
            if cid.is_admin = true{
                for nid in data_state.user_in_group {
                    if nid.uid == id && nid.gid == gid && check_admins_number(gid) > 1 {
                        cid.is_admin = false;
                        return;
                    } 
                }
...
fn leave_group(aid: u64, gid: u64) {
    if check_admins_number(gid) > 1 {
        for cid in data_state.user_in_group {
...
fn remove_group(aid: u64, gid: u64) {
...
fn distr_sec_santas(aid: u64, gid: u64) {
    /* */
    for cg in data_state.group {
        if cg.id == gid {
            cg.is_closed = true;
...
fn check_admins_number(gid: u64) -> u64 {
    let count:u64 = 0;
    for cid in data_state.user_in_group {
